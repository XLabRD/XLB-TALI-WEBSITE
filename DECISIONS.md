...
| DEC-17 | Pricing figures? | OPEN | Displayed price fetched live from Stripe at page load (worker `/price`); static fallback `$148.00 USD` in `founder.json` for no-JS/offline. Real launch figure still unconfirmed — change it in Stripe and the site follows | `workers/checkout/worker.js`, `src/components/Pricing.astro`, `src/content/plans/founder.json` | Editable in Keystatic → Editions (fallback only). 2026-07-01: three placeholder editions replaced by single Founder Limited Edition per author. 2026-07-02: pitch deck targets 2,000 MXN (sensor + hub) and 2,400 MXN/yr service — not published pending author confirmation. 2026-07-26: site price now sourced from the Stripe test price ($148 USD); keep the JSON fallback in sync when the Stripe price changes |
